...
namespace Knowledge4e.Core.Extensions
{
    public static class SqlExtensions
    {
        ...
        {
            var columnToAdd = type.GetColumNames().Where(f => f != "ID").Intersect(columns);
        ...
        public static Dictionary<string, object> MappingDbType(this Type type, object data)
        {
            ...
            var properties = type.GetProperties().Where(k => !excludes.Contains(k.Name));
            var parameters = properties.ToDictionary(k => $"@{"v_" + k.Name}", v => v.GetValue(data));
            return parameters;
        }
    }
}
