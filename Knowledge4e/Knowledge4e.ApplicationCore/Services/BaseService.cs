...
using Knowledge4e.ApplicationCore.Enums;
using Knowledge4e.ApplicationCore.Extensions;
using Knowledge4e.ApplicationCore.Helpers;
using Knowledge4e.ApplicationCore.Interfaces;
using Knowledge4e.Entities;

namespace Knowledge4e.ApplicationCore
{
    /// <summary>
    /// Service dùng chung
    /// </summary>
    ...
    public class BaseService<TEntity> : IBaseService<TEntity> where TEntity : BaseEntity
    {
        ...
        public BaseService(IBaseRepository<TEntity> baseRepository)
        {
            ...
            {
                Data = null,
                Code = Knowledge4e.Entities.Enums.Success,
        ...
        public virtual async Task<ServiceResult> Insert(TEntity entity)
        {
            ...
            if (isValid)
            {
                ...
                if (rowAffects == 0)
                {
                    _serviceResult.Code = Knowledge4e.Entities.Enums.Fail;
        ...
        public async Task<ServiceResult> Update(Guid entityId, TEntity entity)
        {
            ...
            if (model == null)
            {
                _serviceResult.onError(_data: null, _message: null, _code: Knowledge4e.Entities.Enums.NotFound);
                return _serviceResult;
            }
            ...
            if (isValid)
            {
                ...
                if (rowAffects > 0)
                {
                    _serviceResult.Code = Knowledge4e.Entities.Enums.Valid;
                    _serviceResult.Messasge = Properties.Resources.Msg_Success;
                }
                else
                {
                    _serviceResult.Code = Knowledge4e.Entities.Enums.InValid;
                    _serviceResult.Messasge = Properties.Resources.Msg_Failed;
                }
            }
            else
        ...
        public ServiceResult Delete(Guid entityId)
        {
            ...
            if (rowAffects > 0)
            {
                _serviceResult.Code = Knowledge4e.Entities.Enums.Success;
                _serviceResult.Messasge = Properties.Resources.Msg_Success;
            }
            else
            {
        ...
        private async Task<bool> ValidateDulicate(TEntity entity, Guid? id)
        {
            ...
            if (!isValid)
            {
                ...
                _serviceResult.Code = Knowledge4e.Entities.Enums.Duplicate;
                _serviceResult.Messasge = Properties.Resources.Msg_NotValid;
